...
object Task3{
  def main(args: Array[String]) = {
    ...
    (4 to 13) foreach{k=>
      ...
      for(n<-nHashes){
        val signature = signatures(shingles,n)
        (0 to booksAsString.length-1) foreach{i=>
          (0 to booksAsString.length-1) foreach{j=>
            ...
            print(k)
            print(" ")
            val sim = jac_Signature(signature(i),signature(j))
            print(sim)
            println()
          }
        }
  ...
  def jac_Signature(bookASignature:Array[Int],bookBSignature:Array[Int]):Float={
  ...
  def signatures(shingles:Array[Set[String]],nHashes:Int):Array[Array[Int]]={
    ...
    val signatures = (0 to nHashes-1).map((x)=>hash(columns))
    return (0 to columns.length-1).map((x)=>signatures.map((y)=>y(x)).toArray).toArray

  }

  def hash(matrix:Array[Array[Int]]):Array[Int]={
  ...
  def generate_hash(n:Int,len:Int):Array[Array[Int]]={
  ...
  def jac_similarity(bookA:String, bookB:String,k:Int):Float={
    val bookAShingles = kShingles(bookA,k)
    val bookBShingles = kShingles(bookB,k)
    return jaccard_full(bookAShingles,bookBShingles)
  }
  ...
  def jaccard_full(bookAShingles:Set[String],bookBShingles:Set[String]):Float={
